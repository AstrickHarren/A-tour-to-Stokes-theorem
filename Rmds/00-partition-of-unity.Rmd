...
In elementary analysis courses, instructors present proofs of Stokes' theorem in its simple forms first, usually on 2-dimensional rectangles. Then they attempt to decompose domains in $ℝ^2$ into such small rectangles, and argue the integrals over the entire domain is equal to the sum of those over the rectangles. This generalization of local conclusions to their global counterparts is performed formally with the technique shown here known as the partition of unity (definition \@ref(def:partition-of-unity)). 
...
Always remember the support of a function is the closure taken relative to its *domain of definition*. 
...
A function $f: V → I = [0, 1]$ of some open subset $V ⊂ ℝ^n$ is a bump function at $x ∈ V$ supported in $U$ if

1. $f = 1$ in a neighborhood of $x$.^[It makes no difference to assert whether the neighborhood needs be taken relative to $V$ or $ℝ^n$.]
2. $\supp f ⊂ U$.
...
```{theorem, existence-of-bump-function}
 ...
 For all $x ∈ ℝ^n$ and $U, V$ are two of its neighborhoods in $ℝ^n$, there exists a smooth bump function $f: V → I$ at $x$ supported in $U$.
...
3. If $n = 1$, $x = 0$, $U = (-2r, 2r)$, $V = ℝ$ choose any $0 < s < r$ and define
...
4. If $V = ℝ^n$, there is an open ball $B$ with radius $2r$ around $x$ contained in $U$, put

    $$σ(x) = ρ(\abs{x})$$

    gives a smooth bump function that has support in $B$. The function is smooth because $\abs{x}^2$ is smooth (even when $\abs{x}$ itself is not) and $ρ$ is a composition that takes square as its innermost function.

5. Finally, if $V$ is any open subset of $ℝ^n$, put 

    $$η = σ|_V$$
    
    Obviously, the support^[Again, the supports here are taken wrto their corresponding domains of definition]

    $$\supp η = \supp σ|_V ⊂ \supp σ ⊂ U$$
    
    and $η = 1$ in a neighborhood of $x$ relative to $V$ since $V$ is open. 
...
Of course, let $\mathcal R$ be the partition of unity introduced by the theorem such that the support of every $σ ∈ \mathcal R$ is contained in $U_{α}$ for some $α = τ(σ)$ depending on $σ$ in its construction.^[Or one can resort to the Axiom of Choice for such choice function $τ$] Define 
$$
ρ_α = \sum_{τ(σ) = α}σ
$$
1.  By lemma \@ref(lem:closure-of-locally-finite-sets)
    ...
    $$
    In addition, $\{\supp ρ_α\}$ is also locally finite because any $x$ intersecting with infinitely many $\supp ρ_α$ will do so with infinitely many $\bigcup_{τ(σ)=α}\supp σ$ each of which is disjoint from others and contains at least one $\supp σ$ for $\supp ρ_α$ not to be empty.
